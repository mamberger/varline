...
function create_event(event, block) {
    ...
    button.id = event.id
    button.setAttribute('data-title', event.home+' '+event.score+' '+event.away)
    ...
    block.append(button)
    block.innerHTML += '<div class="x-pb-4" style="background: #1cbd55;margin-top: -17px;position: relative;height: 20px !important;font-size: 14px;font-weight: 600 !important;border-radius: 0px 0px 8px 8px;color: #fff;">«Видеообзор матча, лучшие моменты!»</div><br>'
};
