...
module.exports = (window => {
  ...
  const ZuckJS = function (timeline, options) {
    ...
    const optionsDefault = {
      ...
      template: {
        ...
        viewerItem (storyData, currentStoryItem) {
          return `<div class="story-viewer">
                    <div class="head">
                      <div class="left">
                        ${option('backButton') ? '<a class="back"> </a>' : ''}
                        <span class="item-preview">
                      ...
                      <div class="right">
                        ...
                        <a class="close" tabIndex="2"><img width="42" src="https://varbase.ru/apk/x.png"></a>
                      </div>
                    </div>
      ...
      language: {
        unmute: '<img width="20" src="https://varbase.ru/apk/sound.png" style="margin-top: -4px;">   Коснись экрана ',
    ...
    const ZuckModal = () => {
      ...
      return {
        ...
        previous (unmute) {
            const callback = function () {
                const nextStory = zuck.internalData.currentStory;
                const nextStoryTimeLineElement = query(
                    `#${id} [data-id="${nextStory}"]`
                );
                if (nextStoryTimeLineElement) {
                    nextStoryTimeLineElement.classList.add('seen');

                    zuck.data[nextStory].seen = true;
                    zuck.internalData.seenItems[nextStory] = true;

                    saveLocalData('seenItems', zuck.internalData.seenItems);
                    updateStorySeenPosition();
                }

                const stories = query('#zuck-modal .story-viewer.preview');
                moveStoryItem(false);
            };
